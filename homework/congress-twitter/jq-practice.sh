...
echo '6.'
cat data-hold/legislators-current.json | \
 jq '.[] .bio .religion' | sort -rn | uniq -c | tr -d '"' | sort -rn

echo '7.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | cut -d '-' -f 1 | grep -c '2015'

echo '8.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | cut -d '-' -f 1 | uniq | head -n 1

echo '9.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[-1:][0].party' | grep -v 'Republican' | grep -v 'Democrat' -c
...
echo '17.'
cat data-hold/congress-twitter-profiles.json | \
  jq '.[] .followers_count' | uniq | sort -rn | head -n 1
